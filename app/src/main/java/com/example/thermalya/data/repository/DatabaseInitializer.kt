        ...
        CoroutineScope(Dispatchers.Main).launch {
            try {
                ...
                val dipendentiList = if (dipendentiResult.getOrNull()?.isEmpty() == true) {
                    ...
                    // Ricarica per ottenere gli ID
                    userRepo.getAllDipendenti().getOrNull() ?: emptyList()
                } else {
                    android.util.Log.d("DBInit", "Dipendenti già presenti: ${dipendentiResult.getOrNull()?.size}")
                    dipendentiResult.getOrNull() ?: emptyList()
                }

                // 2. CLIENTI (solo se non esistono già)
                val clientiResult = clienteRepo.getAllClienti()
                val clientiList = if (clientiResult.getOrNull()?.isEmpty() == true) {
                    android.util.Log.d("DBInit", "Creazione clienti...")
                    val clienti = createClienti()
                    val clientiListTemp = mutableListOf<Cliente>()
                    clienti.forEach { cliente ->
                        val result = clienteRepo.addCliente(cliente)
                        result.onSuccess { clienteId ->
                            clientiListTemp.add(cliente.copy(id = clienteId))
                            successCount++
                        }
                        result.onFailure { errorCount++ }
                    }
                    android.util.Log.d("DBInit", "Clienti create: ${clientiListTemp.size}")
                    clientiListTemp
                } else {
                    android.util.Log.d("DBInit", "Clienti già presenti: ${clientiResult.getOrNull()?.size}")
                    clientiResult.getOrNull() ?: emptyList()
                }

                // 3. TRATTAMENTI (solo se non esistono già)
                val trattamentiCheck = Firebase.firestore.collection("trattamenti").limit(1).get().await()
                if (trattamentiCheck.isEmpty) {
                    ...
                    android.util.Log.d("DBInit", "Trattamenti già presenti")
                }

                // 4. PRODOTTI (solo se non esistono già)
                val prodottiCheck = Firebase.firestore.collection("prodotti").limit(1).get().await()
                if (prodottiCheck.isEmpty) {
                    ...
                    android.util.Log.d("DBInit", "Prodotti già presenti")
                }

                // 5. APPUNTAMENTI DI TEST (solo se non esistono già)
                val appuntamentiCheck = Firebase.firestore.collection("appuntamenti").limit(1).get().await()
                if (appuntamentiCheck.isEmpty && dipendentiList.isNotEmpty() && clientiList.isNotEmpty()) {
                    ...
                    if (appuntamentiCheck.isEmpty.not()) {
                        android.util.Log.d("DBInit", "Appuntamenti già presenti")
                    } else {
                        android.util.Log.w("DBInit", "Impossibile creare appuntamenti: mancano dipendenti o clienti")
                    }
                }
...
// Repository per Trattamenti
class FirestoreTrattamentoRepository {
    private val db = Firebase.firestore
    private val trattamentiCollection = db.collection("trattamenti")

    suspend fun addTrattamento(trattamento: Trattamento): Result<String> {
        return try {
            val docRef = trattamentiCollection.add(trattamento).await()
    ...
    suspend fun getTrattamento(trattamentoId: String): Result<Trattamento?> {
        return try {
            val document = trattamentiCollection.document(trattamentoId).get().await()
            val trattamento = document.toObject(Trattamento::class.java)?.copy(id = document.id)
            Result.success(trattamento)
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
...
class FirestoreProdottoRepository {
